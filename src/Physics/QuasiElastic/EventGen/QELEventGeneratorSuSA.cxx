...
void QELEventGeneratorSuSA::GenerateNucleon(GHepRecord * event) const
{
    ...
    while(!accept){
        ...
        if(fGeneralizedContactFormalismOffShell && fNuclModel->FermiMomentum() < fNuclModel->Momentum3().Mag()){
          ...
          double Pp = (initial_nucleon_pdg == kPdgProton) ? fPPPairPercentage : fPNPairPercentage;
          RandomGen * rnd = RandomGen::Instance();
          double prob = rnd->RndGen().Rndm();
          int eject_nucleon_pdg = (prob > Pp) ? kPdgNeutron : kPdgProton;

          fNuclModel->SetRecoilPDG(eject_nucleon_pdg);
          bool is_hit_p  = pdg::IsProton(initial_nucleon_pdg);
          bool is_rec_p  = pdg::IsProton(eject_nucleon_pdg);

          int A,Z;
          if((is_hit_p && !is_rec_p) || (!is_hit_p && is_rec_p)){ Z = target_nucleus->Z()-1; }
          if(is_hit_p && is_rec_p){ Z = target_nucleus->Z()-2; }
          if(!is_hit_p && !is_rec_p){ Z = target_nucleus->Z(); }
          A = target_nucleus->A() - 2;

          int ipdgc = pdg::IonPdgCode(A, Z);
          TParticlePDG *fnucleus = PDGLibrary::Instance()->Find(ipdgc);
          double Mf  = fnucleus -> Mass();
          
          double recoil_px = fNuclModel->COMMomentum3().X() - fNuclModel->Momentum3().X();
         ...
         // std::cout << "FM Recoil Px: " << fNuclModel->COMMomentum3().X() << " - " << fNuclModel->Momentum3().X() << " = " << recoil_px << std::endl;
         // std::cout << "FM Recoil Py: " << fNuclModel->COMMomentum3().Y() << " - " << fNuclModel->Momentum3().Y() << " = " << recoil_py << std::endl;
         // std::cout << "FM Recoil Pz: " << fNuclModel->COMMomentum3().Z() << " - " << fNuclModel->Momentum3().Z() <<  " = " << recoil_pz << std::endl;
         ...
         // std::cout << "FM Recoil PDG: " << eject_nucleon_pdg << std::endl;
          ...
          double Mi = target_nucleus->Mass();
          double EN = Mi - sqrt(TMath::Power(fNuclModel->COMMomentum(),2) + TMath::Power(Mf,2)) - recoil_E; 


        //  std::cout << "FM Recoil Mass: " << recoil_M << std::endl;

         // std::cout << "FM ERec: " << recoil_E << std::endl;
        //  std::cout << "FM PCOM: " << fNuclModel->COMMomentum3().Mag() << std::endl;

        //  std::cout << "FM MNuc: " << Mi << std::endl;
        //  std::cout << "FM MRem: " << Mf << std::endl;
          ...
          //   the GHEP record
         /* p4initial_nucleon->SetPx( p3i.Px() );
          p4initial_nucleon->SetPy( p3i.Py() );
          p4initial_nucleon->SetPz( p3i.Pz() );
          p4initial_nucleon->SetE ( EN      );
          */p4initial_nucleon.SetXYZT(p3i.Px(),p3i.Py(),p3i.Pz(),EN);
        ...
        //  std::cout << "FM M:" << p4initial_nucleon.M() << std::endl;
    ...
    if ( fSecondEmitter ) fSecondEmitter -> ProcessEventRecord( event ) ;
...
void QELEventGeneratorSuSA::LoadConfig(void)
{
    ...
    this->GetParamDef("GeneralizedContactFormalismOffShell", fGeneralizedContactFormalismOffShell, true);
