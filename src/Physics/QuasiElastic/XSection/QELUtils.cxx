  ...
  const std::string& binding_mode)
{
  ...
  else if ( binding_mode == "GeneralizedContactFormalismOffShell" ) {
    return kGeneralizedContactFormalismOffShell;
  }
  else {
  ...
  genie::QELEvGen_BindingMode_t hitNucleonBindingMode)
{
  ...
  if ( tgt->IsNucleus() && hitNucleonBindingMode != genie::kOnShell ) {
    ...
    ENi = Mi - std::sqrt( Mf*Mf + p3Ni.Mag2() );
     ...
     if ( hitNucleonBindingMode == genie::kGeneralizedContactFormalismOffShell ){
      
      int nucleon_pdgc = tgt->HitNucPdg();
      
      double Pp = (nucleon_pdgc == kPdgProton) ? .05 : .95;
      RandomGen * rnd = RandomGen::Instance();
      double prob = rnd->RndGen().Rndm();
      int eject_nucleon_pdg = (prob > Pp) ? kPdgNeutron : kPdgProton;

      nucl_model.SetRecoilPDG(eject_nucleon_pdg);
      bool is_hit_p  = pdg::IsProton(nucleon_pdgc);
      bool is_rec_p  = pdg::IsProton(eject_nucleon_pdg);

      int Z;
      if((is_hit_p && !is_rec_p) || (!is_hit_p && is_rec_p)){ Z = tgt->Z()-1; }
      if(is_hit_p && is_rec_p){ Z = tgt->Z()-2; }
      if(!is_hit_p && !is_rec_p){ Z = tgt->Z(); }
      int A = tgt->A() - 2;
      ...
      TVector3 recoil_momentum = nucl_model.COMMomentum3() - p3Ni;
      
      double recoil_M  = PDGLibrary::Instance()->Find(eject_nucleon_pdg)->Mass();
      double recoil_E  = TMath::Sqrt(recoil_momentum.Mag2()+recoil_M*recoil_M);

      ENi = Mi - sqrt(TMath::Power(nucl_model.COMMomentum(),2) + TMath::Power(Mf,2)) - recoil_E; 

      std::cout << "QELU px:" << nucl_model.Momentum3().X() << std::endl;
      std::cout << "QELU py:" << nucl_model.Momentum3().Y() << std::endl;
      std::cout << "QELU pz:" << nucl_model.Momentum3().Z() << std::endl;
      std::cout << "QELU EN:" << ENi << std::endl;
      std::cout << "QELU M:" << p4Ni->M() << std::endl;
